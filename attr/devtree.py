...
def assignValues(content):
	...
	for i in range(1,len(content)):
		...
		if(content[i][0]=='ATTR_HWAS_STATE'):
			...
			for j in range(2,8):#bits 0,1(7,8 from lsb) are reserved, so checking what other bits are set
...
def processFile(file_content):
	'''This function takes the list returned by openFile() as input and returns a '''
	for i in range(len(file_content)):
		file_content[i]=file_content[i].split("\n")#each element of the target list is sent to read() so that it is split into a list of attributes
	...
	return file_content#returns a list iof dictionaries
def ListTargets(tar_list):
	#tar_list=list(sorted(tar_list,key=lambda x : x[x.find('.')+1:(x.find(':'))]))
	for i in tar_list:
		print(i)
...
if('bmc' not in host.stdout.decode('utf-8')):
	uname='service'
	pwd='0penBmc0'
	host='rain111bmc.aus.stglabs.ibm.com'
	...
	#file_content=openFile('output.txt')
else:
...
if(len(sys.argv)>=2):
	if(sys.argv[1]=='-help'):
		...
		sys.argv[1]='-f'
#opt=['-h','-t','-p','-l','-n',"-f"]
#print(type(sys.argv[1]),type(opt[-1]))
#print(sys.argv[1]==opt[-1])
#print(sys.argv[1] not in opt)
if(len(sys.argv)<2 or (len(sys.argv)>=2 and (sys.argv[1] not in ['-h','-t','-p','-l','-n','-f']))):#Goes inside, if there is no option or the option is -j
