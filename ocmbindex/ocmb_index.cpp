...
int main()
{
    ...
    pdbg_for_each_target("ocmb", NULL, target)
    {
        ...
        uint32_t proc = pdbg_target_index(target) / 8;
        ...
        if(chipId == ODYSSEY_CHIP_ID)
        {
            ...
            if( targetPath)
            {
                std::cout << "proc " << proc << " odyocmb pdbg target path is " << targetPath << " index " << std::hex << "0x" << pdbg_target_index(target) << " fapipos " << "0x" << fapi_pos <<  std::endl;
            }
        }
        else
        {
            ...
            if( targetPath)
            {
            std::cout << "proc " << proc << " ocmb pdbg target path is " << targetPath << " index " << std::hex << "0x" << pdbg_target_index(target) << " fapipos " << "0x" << fapi_pos <<  std::endl;
            }
        }
    }
    return 0;
